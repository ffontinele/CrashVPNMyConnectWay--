...
#-----------------------------------------------------------------------------------------------------------
#	DATA:				07 de MarÃ§o de 2017
#	SCRIPT:				ShellBot.sh
#	VERSÃƒO:				6.3.0
#	DESENVOLVIDO POR:	Juliano Santos [SHAMAN]
#	PÃGINA:				http://www.shellscriptx.blogspot.com.br
#	FANPAGE:			https://www.facebook.com/shellscriptx
#	GITHUB:				https://github.com/shellscriptx
# 	CONTATO:			shellscriptx@gmail.com
#
#	DESCRIÃ‡ÃƒO:			ShellBot Ã© uma API nÃ£o-oficial desenvolvida para facilitar a criaÃ§Ã£o de 
#						bots na plataforma TELEGRAM. ConstituÃ­da por uma coleÃ§Ã£o de mÃ©todos
#						e funÃ§Ãµes que permitem ao desenvolvedor:
#
#							* Gerenciar grupos, canais e membros.
#							* Enviar mensagens, documentos, mÃºsicas, contatos e etc.
#							* Enviar teclados (KeyboardMarkup e InlineKeyboard).
#							* Obter informaÃ§Ãµes sobre membros, arquivos, grupos e canais.
#							* Para mais informaÃ§Ãµes consulte a documentaÃ§Ã£o:
#							  
#							https://github.com/shellscriptx/ShellBot/wiki
#
#						O ShellBot mantÃ©m o padrÃ£o da nomenclatura dos mÃ©todos registrados da
#						API original (Telegram), assim como seus campos e valores. Os mÃ©todos
#						requerem parÃ¢metros e argumentos para a chamada e execuÃ§Ã£o. ParÃ¢metros
#						obrigatÃ³rios retornam uma mensagem de erro caso o argumento seja omitido.
#					
#	NOTAS:				Desenvolvida na linguagem Shell Script, utilizando o interpretador de 
#						comandos BASH e explorando ao mÃ¡ximo os recursos built-in do mesmo,
#						reduzindo o nÃ­vel de dependÃªncias de pacotes externos.
#-----------------------------------------------------------------------------------------------------------
...
[description]='API nÃ£o-oficial para criaÃ§Ã£o de bots na plataforma Telegram.'
[version]='6.0'
...
while read _pkg_ _ver_; do
	if command -v $_pkg_ &>/dev/null; then
		if [[ $($_pkg_ --version 2>&1) =~ [0-9]+\.[0-9]+ ]]; then
			...
			printf "%s: erro: '%s' nÃ£o foi possÃ­vel obter a versÃ£o.\n" ${_SHELLBOT_[name]} $_pkg_ 1>&2
			exit 1
		fi
	else
		printf "%s: erro: '%s' o pacote requerido estÃ¡ ausente.\n" ${_SHELLBOT_[name]} $_pkg_ 1>&2
		exit 1
	fi
...
# Desabilita a expansÃ£o de nomes de arquivos (globbing).
set -f

readonly _SHELLBOT_SH_=1					# InicializaÃ§Ã£o
readonly _BOT_SCRIPT_=${0##*/}				# Script
readonly _CURL_OPT_='--silent --request'	# CURL (opÃ§Ãµes)

# Erros
readonly _ERR_TYPE_BOOL_='tipo incompatÃ­vel: suporta somente "true" ou "false".'
readonly _ERR_TYPE_INT_='tipo incompatÃ­vel: suporta somente inteiro.'
readonly _ERR_TYPE_FLOAT_='tipo incompatÃ­vel: suporta somente float.'
readonly _ERR_PARAM_REQUIRED_='opÃ§Ã£o requerida: verique se o(s) parÃ¢metro(s) ou argumento(s) obrigatÃ³rio(s) estÃ£o presente(s).'
readonly _ERR_TOKEN_UNAUTHORIZED_='nÃ£o autorizado: verifique se possui permissÃµes para utilizar o token.'
readonly _ERR_TOKEN_INVALID_='token invÃ¡lido: verique o nÃºmero do token e tente novamente.'
readonly _ERR_BOT_ALREADY_INIT_='aÃ§Ã£o nÃ£o permitida: o bot jÃ¡ foi inicializado.'
readonly _ERR_FILE_NOT_FOUND_='falha ao acessar: nÃ£o foi possÃ­vel ler o arquivo.'
readonly _ERR_DIR_WRITE_DENIED_='permissÃ£o negada: nÃ£o Ã© possÃ­vel gravar no diretÃ³rio.'
readonly _ERR_DIR_NOT_FOUND_='NÃ£o foi possÃ­vel acessar: diretÃ³rio nÃ£o encontrado.'
readonly _ERR_FILE_INVALID_ID_='id invÃ¡lido: arquivo nÃ£o encontrado.'
readonly _ERR_UNKNOWN_='erro desconhecido: ocorreu uma falha inesperada. Reporte o problema ao desenvolvedor.'
readonly _ERR_SERVICE_NOT_ROOT_='acesso negado: requer privilÃ©gios de root.'
readonly _ERR_SERVICE_EXISTS_='erro ao criar o serviÃ§o: o nome do serviÃ§o jÃ¡ existe.'
readonly _ERR_SERVICE_SYSTEMD_NOT_FOUND_='erro ao ativar: o sistema nÃ£o possui suporte ao gerenciamento de serviÃ§os "systemd".'
readonly _ERR_SERVICE_USER_NOT_FOUND_='usuÃ¡rio nÃ£o encontrado: a conta de usuÃ¡rio informada Ã© invÃ¡lida.'
readonly _ERR_VAR_NAME_='variÃ¡vel nÃ£o encontrada: o identificador Ã© invÃ¡lido ou nÃ£o existe.'
readonly _ERR_FUNCTION_NOT_FOUND_='funÃ§Ã£o nÃ£o encontrada: o identificador especificado Ã© invÃ¡lido ou nÃ£o existe.'
readonly _ERR_ARG_='argumento invÃ¡lido: o argumento nÃ£o Ã© suportado pelo parÃ¢metro especificado.'
readonly _ERR_RULE_ALREADY_EXISTS_='falha ao definir: o nome da regra jÃ¡ existe.'
readonly _ERR_HANDLE_EXISTS_='erro ao registar: jÃ¡ existe um handle vinculado ao callback'
readonly _ERR_CONNECTION_='falha de conexÃ£o: nÃ£o foi possÃ­vel estabelecer conexÃ£o com o Telegram.'
...
MessageError()
{
	...
	# A variÃ¡vel 'BASH_LINENO' Ã© dinÃ¢mica e armazena o nÃºmero da linha onde foi expandida.
	# Quando chamada dentro de um subshell, passa ser instanciada como um array, armazenando diversos
	# valores onde cada Ã­ndice refere-se a um shell/subshell. As mesmas caracteristicas se aplicam a variÃ¡vel
	# 'FUNCNAME', onde Ã© armazenado o nome da funÃ§Ã£o onde foi chamada.
	
	# Obtem o Ã­ndice da funÃ§Ã£o na hierarquia de chamada.
	[[ ${FUNCNAME[1]} == CheckArgType ]] && i=2 || i=1
	
	# LÃª o tipo de ocorrÃªncia.
	...
	# Finaliza script/thread em caso de erro interno, caso contrÃ¡rio retorna 1
	${assert:-false} && exit 1 || return 1
}

CheckArgType()
{
	# CheckArgType recebe os dados da funÃ§Ã£o chamadora e verifica
	# o dado recebido com o tipo suportado pelo parÃ¢metro.
	# Ã‰ retornado '0' para sucesso, caso contrÃ¡rio uma mensagem
	# de erro Ã© retornada e o script/thread Ã© finalizado com status '1'.
	case $1 in
...
ShellBot.init()
{
	...
	# Verifica se o bot jÃ¡ foi inicializado.
	...
	# Define os parÃ¢metros posicionais
	eval set -- "$param"
				...
				-t|--token)
	   			...
	   			-m|--monitor)
					...
					monitor=true
	   				...
	   				;;
				-f|--flush)
					# Define a FLAG flush para o mÃ©todo 'ShellBot.getUpdates'. Se ativada, faz com que
					# o mÃ©todo obtenha somente as atualizaÃ§Ãµes disponÃ­veis, ignorando a extraÃ§Ã£o dos
					# objetos JSON e a inicializaÃ§Ã£o das variÃ¡veis.
					flush=true
					shift
					;;
				-s|--service)
    ...
    # Um mÃ©todo simples para testar o token de autenticaÃ§Ã£o do seu bot. 
    # NÃ£o requer parÃ¢metros. Retorna informaÃ§Ãµes bÃ¡sicas sobre o bot em forma de um objeto UsuÃ¡rio.
    ShellBot.getMe()
    {
    	# Chama o mÃ©todo getMe passando o endereÃ§o da API, seguido do nome do mÃ©todo.
    	jq_obj=$(curl $_CURL_OPT_ GET $_API_TELEGRAM_/${FUNCNAME#*.})

		# Verifica o status de retorno do mÃ©todo
    ...
    # SHELLBOT (FUNÃ‡Ã•ES)
	# Inicializa as funÃ§Ãµes para chamadas aos mÃ©todos da API do telegram.
    ...
    ShellBot.InlineKeyboardButton()
    {
    	...
    	do
    		case $1 in
    			-b|--button)
    				# Ponteiro que recebe o endereÃ§o de "button" com as definiÃ§Ãµes
    				# da configuraÃ§Ã£o do botÃ£o inserido.
    ...
    ShellBot.InlineKeyboardMarkup()
    {
    	...
    	do
    		case $1 in
    			-b|--button)
    				# Ponteiro que recebe o endereÃ§o da variÃ¡vel "teclado" com as definiÃ§Ãµes
    				# de configuraÃ§Ã£o do botÃ£o inserido.
					CheckArgType var "$1" "$2"
    				__button="$2"
    				shift 2
    				;;
    	...
    	# Constroi a estrutura dos objetos + array keyboard, define os valores e salva as configuraÃ§Ãµes.
    	# Por padrÃ£o todos os valores sÃ£o 'false' atÃ© que seja definido.
    ...
    # Cria objeto que representa um teclado personalizado com opÃ§Ãµes de resposta
    ShellBot.ReplyKeyboardMarkup()
    {
    	# VariÃ¡veis locais
    	local __button __resize_keyboard __on_time_keyboard __selective __keyboard
    	
    	# LÃª os parÃ¢metros da funÃ§Ã£o.
    	local __param=$(getopt 	--name "$FUNCNAME" \
							 	--options 'b:r:t:s:' \
    						 	--longoptions 'button:,
    										resize_keyboard:,
    	...
    	# Transforma os parÃ¢metros da funÃ§Ã£o em parÃ¢metros posicionais
    	#
    	# Exemplo:
    	...
    	# Aguarda leitura dos parÃ¢metros
    	while :
    	do
    		# LÃª o parÃ¢metro da primeira posiÃ§Ã£o "$1"; Se for um parÃ¢metro vÃ¡lido,
    		# salva o valor do argumento na posiÃ§Ã£o '$2' e desloca duas posiÃ§Ãµes a esquerda (shift 2); Repete o processo
    		# atÃ© que o valor de '$1' seja igual '--' e finaliza o loop.
    		case $1 in
    			-b|--button)
					CheckArgType var "$1" "$2"
    				__button=$2
    	...
    	# Imprime mensagem de erro se o parÃ¢metro obrigatÃ³rio for omitido.
    	[[ $__button ]] || MessageError API "$_ERR_PARAM_REQUIRED_" "[-b, --button]"
    ...
    ShellBot.sendMessage()
    {
    	...
    	# Chama o mÃ©todo da API, utilizando o comando request especificado; Os parÃ¢metros 
    	# e valores sÃ£o passados no form e lidos pelo mÃ©todo. O retorno do mÃ©todo Ã© redirecionado para o arquivo 'update.Json'.
    	# VariÃ¡veis com valores nulos sÃ£o ignoradas e consequentemente os respectivos parÃ¢metros omitidos.
    	jq_obj=$(curl $_CURL_OPT_ POST $_API_TELEGRAM_/${FUNCNAME#*.} \
									${chat_id:+-d chat_id="$chat_id"} \
									${text:+-d text="$text"} \
    ...
    # FunÃ§Ã£o para reencaminhar mensagens de qualquer tipo.
    ...
    # Utilize essa funÃ§Ã£o para enviar fotos.
    ShellBot.sendPhoto()
    {
    	# VariÃ¡veis locais
    	...
    	do
    		case $1 in
    			...
    			-p|--photo)
    				...
    				;;
    			-t|--caption)
    				# Limite mÃ¡ximo de caracteres: 200
					caption=$(echo -e "$2")
    				shift 2
    				;;
				-m|--parse_mode)
    ...
    # Utilize essa funÃ§Ã£o para enviar arquivos de audio.
    ShellBot.sendAudio()
    {
    	# VariÃ¡veis locais
    ...
    # Utilize essa funÃ§Ã£o para enviar documentos.
    ...
    # Utilize essa funÃ§Ã£o para enviat stickers
...
_EOF
    ...
    # FunÃ§Ã£o para enviar arquivos de vÃ­deo.
    ShellBot.sendVideo()
    {
    	# VariÃ¡veis locais
    ...
    # FunÃ§Ã£o utilizada para enviar uma localidade utilizando coordenadas de latitude e longitude.
    ...
    # FunÃ§Ã£o utlizada para enviar detalhes de um local.
    ShellBot.sendVenue()
    {
    	# VariÃ¡veis locais
    ...
    # Utilize essa funÃ§Ã£o para enviar um contato + numero
    ...
    # Utilize essa funÃ§Ã£o para obter as fotos de um determinado usuÃ¡rio.
    ShellBot.getUserProfilePhotos()
    {
    	...
    	# Verifica se ocorreu erros durante a chamada do mÃ©todo	
    	MethodReturn $jq_obj || MessageError TG $jq_obj
    ...
    # FunÃ§Ã£o para listar informaÃ§Ãµes do arquivo especificado.
    ShellBot.getFile()
    {
    	# VariÃ¡veis locais
    ...
    # Essa funÃ§Ã£o kicka o usuÃ¡rio do chat ou canal. (somente administradores)
    ...
    # Utilize essa funÃ§Ã£o para remove o bot do grupo ou canal.
    ShellBot.leaveChat()
    {
    	# VariÃ¡veis locais
    	local chat_id jq_obj
	...
	ShellBot.downloadFile()
	{
		...
		# Gera o nome do arquivo anexando o horÃ¡rio de criaÃ§Ã£o.
		file=file$(date +%d%m%Y%H%M%S%N)${ext:-.dat}

		# Executa o download da uri especificada e retorna um objeto json
		# com as informaÃ§Ãµes do processo. Se tiver sucesso o diretÃ³rio de
		# destino Ã© retornado, caso contrÃ¡rio uma mensagem de erro Ã© apresentada.
		if wget -qO "$dir/$file" "$uri/$file_path"; then
			...
			rm -f "$dir/$file" 2>/dev/null # Remove arquivo invÃ¡lido.
		fi
	...
	ShellBot.ChatPermissions()
	{
		...
		# Retorna o objeto de permissÃµes.
		echo "{${json%,}}"
	...
	ShellBot.manageRules()
	{
		...
		for ((i=0; i < _BOT_RULES_INDEX_; i++)); do
			...
			# define a expressÃ£o padrÃ£o.
			# Captura os grupos contidos no padrÃ£o, separando o
	   		# operador de negaÃ§Ã£o '!' (se presente) para determinar o 
			# tratamento de valiÃ§Ã£o do intervalo.
			#
			# Exemplo 1:
			#              
			#       BASH_REMATCH[2]
			...
			for file in ${match//|/ }; do
				...
				# LÃª os usuÃ¡rios removendo os comentÃ¡rios complementares
			...
			# Anexa tipo da aÃ§Ã£o. (se presente)
			...
			# Chama a funÃ§Ã£o passando os argumentos posicionais. (se existir)
			...
			while [[ $stdout ]]; do
				...
				# Atualiza o buffer de saÃ­da.
				stdout=${stdout:4096}
			
				# Reenvia aÃ§Ã£o se ainda houver dados.	
    ...
    ShellBot.getUpdates()
    {
		...
		# Define os parÃ¢metros da funÃ§Ã£o
		local param=$(getopt 	--name "$FUNCNAME" \
								--options 'o:l:t:a:' \
								--longoptions 'offset:,
												limit:,
    	...
    	do
    		case $1 in
    			...
    			-a|--allowed_updates)
    				...
    				;;
    			--)
    				# Se nÃ£o houver mais parÃ¢metros
    				shift 
    				break
    				;;
    		esac
		...
		# Limpa as variÃ¡veis inicializadas.
		unset $_VAR_INIT_; _VAR_INIT_=
		
		# Se hÃ¡ atualizaÃ§Ãµes.
    	[[ $(jq -r '.result|length' <<< $jq_obj) -eq 0 ]] && return 0
	
		# Se o mÃ©todo 'ShellBot.getUpdates' for invocado a partir de um subshell,
		# as atualizaÃ§Ãµes sÃ£o retornadas em um estrutura de dados json, o mÃ©todo
		# Ã© finalizado e variÃ¡veis nÃ£o sÃ£o inicializadas.
	...
	# Retorna informaÃ§Ãµes do bot.
	MethodReturn $jq_obj

   	return $?
}

# FunÃ§Ãµes (somente leitura)
